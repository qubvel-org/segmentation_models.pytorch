...
from .decoder import AttentionUnetDecoder


class AttentionUnet(SegmentationModel):
    """
    Attention U-Net is a fully convolutional neural network architecture designed for semantic image segmentation.
    It extends the original U-Net by incorporating attention gates in the decoder, which help the network focus on
    relevant spatial regions while suppressing irrelevant background information.

    It consists of two main parts:

    1. An encoder (downsampling path) that extracts increasingly abstract features
    2. A decoder (upsampling path) with attention gates that selectively emphasize informative features from the encoder

    The key is the use of attention-based skip connections between corresponding encoder and decoder layers.
    These connections allow the decoder to access relevant fine-grained details from earlier encoder layers while
    filtering out irrelevant information, which improves segmentation accuracy, particularly in complex or cluttered scenes.
    ...
    Args:
        ...
        decoder_use_norm: Specifies normalization between Conv2D and activation.
            ...
            **Example**:
            ...
            ```
        decoder_attention_type: Attention module used after convolution in decoder of the model.
    ...
    Returns:
        ``torch.nn.Module``: **Attention U-Net**
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        self.name = "attentionunet-{}".format(encoder_name)
        self.initialize()
