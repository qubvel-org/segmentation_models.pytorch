...
def get_stats(
    ...
    if torch.is_floating_point(output) and threshold is None:
        raise ValueError(f"Output should be one of the integer types if ``threshold`` is not None, " + \
            f"got {output.dtype}.")
        
    if torch.is_floating_point(output) and mode == "multiclass":
        raise ValueError(f"For ``multiclass`` mode ``target`` should be one of the integer types, " + \
            f"got {output.dtype}.")

    if mode not in {"binary", "multiclass", "multilabel"}:
        raise ValueError(f"``mode`` should be in ['binary', 'multiclass', 'multilabel'], " + \
            f"got mode={mode}.")
    ...
    if output.shape != target.shape:
        raise ValueError(f"Dimensions should match, but ``output`` shape is not equal to ``target`` " + \
...
def fbeta_score(tp, fp, fn, tn, beta: float = 1.0, reduction: Optional[str] = None,
...
def f1_score(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
    """F1 score"""
    return _compute_metric(
...
def accuracy(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
    """Accuracy"""
    return _compute_metric(
...
def sensitivity(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def specificity(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def positive_predictive_value(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def negative_predictive_value(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def false_negative_rate(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def false_positive_rate(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def false_discovery_rate(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def false_omission_rate(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def positive_likelihood_ratio(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
...
def negative_likelihood_ratio(tp, fp, fn, tn, reduction: Optional[str] = None,
    class_weights: Optional[List[float]] = None, zero_division: Union[str, float] = 1.0,
) -> torch.Tensor:
